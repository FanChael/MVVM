...
*/
public class HlIndicatorTablayout extends TabLayout {
    private int text_size, text_selected_size;

    public HlIndicatorTablayout(@NonNull Context context) {
        super(context);
        init(context, null);
    }

    public HlIndicatorTablayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public HlIndicatorTablayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void setTableTitles(List<String> stringList){
        ...
        for (int i = 0; i < getTabCount(); ++i) {
            ...
            if (null != view) {
                // 布局信息
                TextView textView = view.findViewById(R.id.tid_textMsg);
                // 默认第0个选中的话，加粗
                if (0 == i) {
